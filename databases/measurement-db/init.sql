...
-- ============================================================================
-- ANOMALY DETECTION: Trigger and notification function for analyze-service
-- ============================================================================

-- Function to notify the analyze-service when a new measurement is inserted
CREATE OR REPLACE FUNCTION notify_new_measurement()
RETURNS TRIGGER AS $$
BEGIN
    -- Send notification with measurement details as JSON
    PERFORM pg_notify(
        'new_measurement',
        json_build_object(
            'id', NEW.id,
...
-- Trigger to execute notification function after each insert
DROP TRIGGER IF EXISTS trigger_notify_new_measurement ON measurements;
