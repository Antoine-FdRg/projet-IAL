...
describe('EnvService Unit Tests', () => {
    ...
    describe('verifyQueueEnvVars', () => {
        it('should not throw when all required queue env vars are set', () => {
            process.env.CLEANER_PRODUCER_QUEUE = 'producer.queue';
            process.env.CLEANER_CONSUME_QUEUE = 'consume.queue';

            expect(() => EnvService.verifyQueueEnvVars()).not.toThrow();
        });

        it('should throw error when CLEANER_PRODUCER_QUEUE is missing', () => {
            delete process.env.CLEANER_PRODUCER_QUEUE;
            process.env.CLEANER_CONSUME_QUEUE = 'consume.queue';

            expect(() => EnvService.verifyQueueEnvVars()).toThrow(
                'Veuillez définir la variable d\'environnement CLEANER_PRODUCER_QUEUE'
            );
        });

        it('should throw error when CLEANER_CONSUME_QUEUE is missing', () => {
            process.env.CLEANER_PRODUCER_QUEUE = 'producer.queue';
            delete process.env.CLEANER_CONSUME_QUEUE;

            expect(() => EnvService.verifyQueueEnvVars()).toThrow(
                'Veuillez définir la variable d\'environnement CLEANER_CONSUME_QUEUE'
            );
        });
    });
    ...
    describe('getConsumeQueue', () => {
        it('should return CLEANER_CONSUME_QUEUE when set', () => {
            process.env.CLEANER_CONSUME_QUEUE = 'test.consume';
    ...
    describe('getProducerQueue', () => {
        it('should return CLEANER_PRODUCER_QUEUE when set', () => {
